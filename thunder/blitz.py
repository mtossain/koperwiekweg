...
def handle_interrupt(channel):
 ...
 if reason == 0x01:
  ...
  print (now + ": Noise level too high - adjusting to level %s") % noise
  f = open("/var/log/noise.log","a+")
  f.write((now + ": Noise level too high - adjusting to level %s") % noise + "\n")
  f.close()
 elif reason == 0x04:
...
while True:
 ...
 if counter > 10:
  if noise > 0:
    ...
    sensor.calibrate(tun_cap=0x07)
    print (now + ": Noise floor reduced to: %s") % noise
    f = open("/var/log/noise.log","a+")
    f.write((now + ": Noise floor reduced to: %s") % noise + "\n") 
    f.close()
    counter = 0
